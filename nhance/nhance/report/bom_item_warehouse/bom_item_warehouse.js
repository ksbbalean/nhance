    ...
    "filters": [{
		...
		console.log("on_change....docName"+docName);
				...
				console.log("productionBOM::"+productionBOM);
                                var status = getBOMStatus(productionBOM);
                                if (status.is_active == 0) {
                                    frappe.msgprint(__("BOM Is Not Active " + docId));
                                }else{
...
function makeStockOperations(report, status, flag) {
    ...
    if (docName == "Project") {
        ...
        bomItemsCalculation(filters, flag, docName);
    }
}
...
function getStockRequistionBOMItemQty(bom, item_code, docName) {
...
function getProductionOrderQty(productionOrder) {
    var qty = 0;
    frappe.call({
        method: 'frappe.client.get_value',
        args: {
...
function bomItemsCalculation(filters, flag, docName) {
    if (docName == "BOM"){
    reference_no = filters.docIds;
    }else{
    reference_no = filters.production_bom_hidden;
    }
    warehouse = ""; 
    var bom_qty = 0;
    var productionOrderQty = 0;
    var workflowStatus = "";
    var productionOrder = filters.docIds;
    ...
    if (bomItemsList.length != 0) {
        ...
        for (var i = 0; i < bomItemsList.length; i++) {
            ...
            bom_item_qty = bomItemsList[i].bom_item_qty;
	    if(docName == "BOM"){
            sumOfItemQty = getStockRequistionBOMItemQty(reference_no, bomItemCode, docName);
	    differnce = ((bom_qty * bom_item_qty) / bom_qty) - (sumOfItemQty);
	    }else{
	    reference_no = productionOrder;
	    sumOfItemQty = getStockRequistionBOMItemQty(productionOrder, bomItemCode, docName);
	    differnce = ((productionOrderQty / bom_qty) * bom_item_qty) - (sumOfItemQty);
	    }
            if (sumOfItemQty == null) {
                sumOfItemQty = 0;
            }
...
function getProductionBOM(docId){
    var bomNo = "";
    frappe.call({
        method: 'frappe.client.get_value',
        args: {
